...
function removeLastOccurrences(str, value) {
  ...
  return str.slice(0, index) + str.slice(l + index, str.length);
}
...
function formatTime(minutes, seconds) {
  const newM = minutes.toString().padStart(2, '0');
...
function isPalindrome(str) {
  const oldStr = str.replace(/[^A-Za-z]/g, '').toLowerCase();
...
function findLongestWord(sentence) {
  const newStr = sentence.split(' ').sort((a, b) => b.length - a.length);
  return newStr[0];
}
...
function reverseWords(str) {
  ...
  return newStr.map((e) => e.split('').reverse().join('')).join(' ');
}
